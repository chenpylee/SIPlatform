...
$(document).ready(function(){
    $( "#supplierForm" ).submit(function( event ) {
        ...
        if(goods_name.trim()=="")
        {
            errorMsg="-［通用信息］商品名称不能为空\n";
            hasError=true;
        }
        if(cat_id==0)
        {
            errorMsg=errorMsg+"-［通用信息］商品分类必须选择\n";
            hasError=true;
        }
        /**检查价格是否为数值**/
        ...
        {
            hasError=true;
            errorMsg=errorMsg+"-［通用信息］本店售价不能为空，且必须为数值\n";
            $('#shop_price').val('0');
        }
        if(isNaN(market_price))
        {
            hasError=true;
            errorMsg=errorMsg+"-［通用信息］市场售价不能为空，且必须为数值\n";
            $('#market_price').val('0');
        }
        ...
        if (!isNaN(goods_number)) {
            if (parseInt(goods_number)==parseFloat(goods_number)) {
                ...
                hasError=true;
                errorMsg=errorMsg+"-［通用信息］商品库存数量必须为大于0的整数\n";
            }
        } else {
            hasError=true;
            errorMsg=errorMsg+"-［通用信息］商品库存数量必须为大于0的整数\n";
        ...
        {
            hasError=true;
            errorMsg=errorMsg+"-［商品相册］请添加至少一张商品图片\n";
        }
        if(hasError)
        {
            alert(errorMsg);
